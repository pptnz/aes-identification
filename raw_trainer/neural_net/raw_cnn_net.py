...
w1 = tf.get_variable(name='w1', shape=[5, 5, 1, 2], dtype=data_type, initializer=tf.contrib.keras.initializers.he_normal())
h1 = tf.nn.conv2d(input_tensor_reshaped, w1, strides=[1, 1, 1, 1], padding='SAME')
h1_pool = tf.nn.max_pool(h1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

w2 = tf.get_variable(name='w2', shape=[5, 5, 2, 4], dtype=data_type, initializer=tf.contrib.keras.initializers.he_normal())
h2 = tf.nn.conv2d(h1_pool, w2, strides=[1, 1, 1, 1], padding='SAME')
h2_pool = tf.nn.max_pool(h2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

w3 = tf.get_variable(name='w3', shape=[5, 5, 4, 8], dtype=data_type, initializer=tf.contrib.keras.initializers.he_normal())
...
w4 = tf.get_variable(name='w4', shape=[5, 5, 8, 16], dtype=data_type, initializer=tf.contrib.keras.initializers.he_normal())
...
w5 = tf.get_variable(name='w5', shape=[5, 5, 16, 32], dtype=data_type, initializer=tf.contrib.keras.initializers.he_normal())
...
w6 = tf.get_variable(name='w6', shape=[128, 16], dtype=data_type, initializer=tf.contrib.keras.initializers.he_normal())
b6 = tf.Variable(tf.truncated_normal([16], dtype=data_type, stddev=0.1))
h6 = tf.nn.tanh(tf.matmul(h5_flat, w6) + b6)
h6_drop = tf.nn.dropout(h6, keep_prob)

w7 = tf.Variable(tf.zeros([16, num_groups], dtype=data_type))
b7 = tf.Variable(tf.zeros([num_groups], dtype=data_type))
