...
w1 = tf.get_variable(name='w1', shape=[7, 7, 1, 16], initializer=tf.contrib.keras.initializers.he_normal())
h1 = tf.nn.conv2d(input_tensor_reshaped, w1, strides=[1, 1, 1, 1], padding='SAME')
h1_pool = tf.nn.max_pool(h1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

w2 = tf.get_variable(name='w2', shape=[5, 5, 16, 64], initializer=tf.contrib.keras.initializers.he_normal())
h2 = tf.nn.conv2d(h1_pool, w2, strides=[1, 1, 1, 1], padding='SAME')
h2_pool = tf.nn.max_pool(h2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

w3 = tf.get_variable(name='w3', shape=[3, 3, 64, 256], initializer=tf.contrib.keras.initializers.he_normal())
...
w4 = tf.get_variable(name='w4', shape=[3, 3, 256, 512], initializer=tf.contrib.keras.initializers.he_normal())
...
w5 = tf.get_variable(name='w5', shape=[3, 3, 512, 512], initializer=tf.contrib.keras.initializers.he_normal())
h5 = tf.nn.conv2d(h4_pool, w5, strides=[1, 1, 1, 1], padding='SAME')
h5_flat = tf.reshape(h5, [-1, 8192])

w6 = tf.get_variable(name='w6', shape=[8192, 1024], initializer=tf.contrib.keras.initializers.he_normal())
...
w7 = tf.get_variable(name='w7', shape=[1024, 16], initializer=tf.contrib.keras.initializers.he_normal())
